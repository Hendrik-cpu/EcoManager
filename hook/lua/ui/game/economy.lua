...
function CreateUI()
    ...
    local function CreateResourceGroup(warningBitmap)
        ...
        group.overflow = UIUtil.CreateText(group, '', 18, UIUtil.bodyFont)
        group.overflow:SetDropShadow(true)
...
function _BeatFunction()
    ...
    if options.gui_smart_economy_indicators == 1 then
        ...
        local function DisplayEconData(controls, tableID, viewPref)
            local function FormatRateString2(RateVal, StoredVal, IncomeAvg, ActualAvg, RequestedAvg)
                
                --local retRateStr = string.format('%+d', math.min(math.max(RateVal, -99999999), 99999999))
                
                local retRateStr = unum(math.min(math.max(RateVal, -99999999), 99999999), 1)
        ...
        -- SET RATE/EFFICIENCY COLOR
            ...
            if(tableID == 'ENERGY') then
                overflow = -getCurrentThrottle()
            end

            if(overflow ~= 0) then
                local color = 'ffb7e75f'
                controls.overflow:Show()
                controls.overflow:SetText(unum(overflow, 1))
                if(overflow < 0) then
                    color = 'red'
                end

                controls.overflow:SetColor(color)
            else
                controls.overflow:Hide()
            end
