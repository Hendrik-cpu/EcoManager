local function getResourceStats(prefix, info)
    local consumed = info[prefix .. 'Consumed']
    local produced = info[prefix .. 'Produced']
    local v = produced - consumed
    local str = string.format('%+d', v)
    local add = ''

    if(produced > 0 and consumed > 0) then
        add = string.format('%s %+d+%d', add, -consumed, produced)
    end

    if(add ~= '') then
        str = string.format('%s (%s)', str, add)
    end
    ...
    return str, UIUtil.UIFile('/game/unit_view_icons/' .. prefix .. '.dds'), color
end

statFuncs[1] = function(info)
    if info.massProduced > 0 or info.massRequested > 0 then
        return getResourceStats('mass', info)
...
statFuncs[2] = function(info)
    if info.energyProduced > 0 or info.energyRequested > 0 then
        return getResourceStats('energy', info)
    else
        return false
    end
end
            ...
            if statFuncs[i](info, bp) then
                if i == 1 or i == 2 then
                    ...
                    if(value) then
                        ...
                        controls.statGroups[i].value:SetColor(color)
                    end
                elseif i == 3 then
                    local value, iconType, color = statFuncs[i](info, bp)
                    controls.statGroups[i].value:SetText(value)
    ...
    if options.gui_detailed_unitview != 0 then
        if info.blueprintId != 'unknown' then
            ...
            if info.userUnit != nil and info.userUnit:GetBuildRate() >= 2 then
                ...
                controls.BuildrateIcon:SetTexture('/mods/EM/textures/wrench.dds')
...
function CreateUI()
    ...
    if options.gui_detailed_unitview != 0 then
        ...
        controls.BuildrateIcon = Bitmap(controls.bg)
    end
